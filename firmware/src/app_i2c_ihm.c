...
void APP_I2C_IHM_Initialize(void)
{
    ...
    appIhmData.transferStatus = TRANSFER_STATUS_ERROR;
...
static void APP_I2C_IHM_I2CCallback(uintptr_t context)
{
    ...
    if(I2C1_ErrorGet() == I2C_ERROR_NONE)
    {
        ...
        {
            appIhmData.transferStatus = TRANSFER_STATUS_SUCCESS;
        }
    }
    else
    {
...
void APP_I2C_IHM_Tasks( void )
{
    ...
    switch (appIhmData.state)
    {
        case I2C_STATE_STATUS_VERIFY:
            ...
            appIhmData.transferStatus = TRANSFER_STATUS_IN_PROGRESS;
            I2C1_Write(ARDUINO_IHM_ADDR, &appIhmData.ackData, ACK_DATA_LENGTH);

            appIhmData.state = I2C_STATE_READ_MOVEMENT;
            break;

        case I2C_STATE_READ_MOVEMENT:
            
            if (appIhmData.transferStatus == TRANSFER_STATUS_SUCCESS && appIhmData.ihmStatus == IHM_INT_STATE)
            {
            ...
            else if (appIhmData.transferStatus == TRANSFER_STATUS_ERROR)
            {
        ...
        case I2C_STATE_WAIT_READ_COMPLETE:
            ...
            if (appIhmData.transferStatus == TRANSFER_STATUS_SUCCESS)
            {
